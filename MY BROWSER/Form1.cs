...
namespace MY_BROWSER
{
	public partial class mainPage : Form
	{
        ...
        private ColorDialog renkDialog = new ColorDialog();
		...
		private void Form1_Load(object sender, EventArgs e)
		{
            ...
            {
                this.BackColor = ColorTranslator.FromHtml(Properties.Settings.Default.ArkaPlanRengi);
            }

            // Önceki seçili öğeyi geri yükle
            if (!string.IsNullOrEmpty(Properties.Settings.Default.ComboBoxSecim))
			{
				backrondSelected.SelectedItem = Properties.Settings.Default.ComboBoxSecim;
			}
			else if (backrondSelected.Text == "")
			{
				backrondSelected.SelectedItem = "Default";
			}
			else if (backrondSelected.Text == "nulls")
			{
		...
		private void Form1_Resize(object sender, EventArgs e)
		{
			...
			panel2.Top = 38; // Paneli en üste sabitle
			...
			BTNminimaze.Left = this.ClientSize.Width - BTNminimaze.Width - 110;

			Btnfs.Top = topYuk;
			Btnfs.Left = this.ClientSize.Width - Btnfs.Width - 60;

			pncBTN.Top = topYuk;
			pncBTN.Left = this.ClientSize.Width - pncBTN.Width - 60;
			...
			PNLtema.Top = 83;
            PNLtema.Left = this.ClientSize.Width - PNLtema.Width - 295;

            gecmisPanel.Top = 209;
            gecmisPanel.Left = this.ClientSize.Width - gecmisPanel.Width - 295;

            bilgiPanel.Top = 468;
            bilgiPanel.Left = this.ClientSize.Width - bilgiPanel.Width - 295;
		...
		{   
            ...
            {
                // Eğer tam ekran modundaysa, pencereyi normale döndür
                if (this.WindowState == FormWindowState.Maximized)
                {
                    this.WindowState = FormWindowState.Normal;
                }
        ...
        {
            gecmisPanel.Visible = !gecmisPanel.Visible;
        ...
        {	
			if(txtUrl.Text == "Google'da Arayın Veya Bir Url Girin")
			{
                txtUrl.Text = " ";
			}
			else
        ...
        private void gecmisList_DoubleClick(object sender, EventArgs e)
        {
			gecmisGirme();
        }

		private void gecmisGirme()
		{
            // ListBox'ta tıklanan öğe
			string secilenMetin = gecmisList.SelectedItem?.ToString();

            // Eğer bir öğe seçildiyse, TextBox'a yapıştırıyoruz
            if (!string.IsNullOrEmpty(secilenMetin))
            {
				dataİnput.Text = secilenMetin;
        ...
        {
            PNLtema.Visible = !PNLtema.Visible;
        }

        private void backrondSelected_SelectedIndexChanged(object sender, EventArgs e)
        {
			if(backrondSelected.Text == "Default")
			{
				this.BackColor = SystemColors.ActiveCaption;
				this.BackgroundImage = null;

                // Kullanıcı seçim yaptığında kaydet
                Properties.Settings.Default.ComboBoxSecim = backrondSelected.SelectedItem.ToString();
                Properties.Settings.Default.Save();

            }
            else if(backrondSelected.Text == "Orman")
			{
				this.BackgroundImage = Properties.Resources.ForestBC;
        ...
        private void resimSecims_Click(object sender, EventArgs e)
        {
            PNLresimSecim.Visible = !PNLresimSecim.Visible;
        }

        private void colorSecims_Click(object sender, EventArgs e)
        {
            

            if (renkDialog.ShowDialog() == DialogResult.OK)
            {
                this.BackColor = renkDialog.Color; // Seçilen rengi uygula

                // Rengi HTML formatında string olarak kaydet
                Properties.Settings.Default.ArkaPlanRengi = ColorTranslator.ToHtml(renkDialog.Color);
                Properties.Settings.Default.Save(); // Ayarları kaydet


                this.BackgroundImage = null;
				if (backrondSelected.Text != "nulls")
				{
					backrondSelected.SelectedItem = "nulls";
        ...
        private void gecmisList_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void verionLBL_Click(object sender, EventArgs e)
        {
        ...
        private void hakkındaLBL_Click(object sender, EventArgs e)
        {
